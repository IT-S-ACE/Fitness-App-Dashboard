...
const AddExerciseToType = () => {
    ...
    const [privatee, setPrivatee] = useState(0)

    const { mutate: addExerciseToType } = useAddNewExerciseToType();
    ...
    //  check the key and add (private , exercise_type_id )   
    const handleAddNewExerciseToType = (e) => {
        e.preventDefault();
        ...
        formData.append('exercise_type_id', type);
        formData.append('private', privatee);
        ...
        addExerciseToType(formData);
    };


    const { data: allCategory } = useAllCategoryData()

    const { data: allType } = useAllTypeData()
                ...
                <div className="m-2 md:m-10 p-2 md:p-10 bg-white rounded-3xl drop-shadow-2xl backdrop-filter bg-opacity-95">
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-1">
                                ...
                                {allCategory?.data?.map((categ) => (
                                    <div key={categ.id} className="px-5 cursor-pointer flex justify-between border-2 border-gray-500 rounded-xl m-2 bg-gray-300" onClick={() => handleCATEGORYCheckbox(categ.id)}>
                                        <div className='flex font-extrabold my-2'>
                                            <img src={`https://de68-138-199-7-163.ngrok-free.app/Uploads/${categ.image}`} alt="Category" className='w-10 h-10 rounded-full' />
                        ...
                        <div className="grid grid-cols-2 gap-1">
                            ...
                            <div className=" border-1 rounded-3xl shadow-2xl flex overflow-x-scroll h-64">
                            ...
                            <div>
                                <div>
                                    <p className="flex ml-2">Type</p>
                                    <div className="mt-2">
                                        <DropDownID dropLabel="Type" items={allType?.data} setSelectedID={setType} />
                                    </div>
                                </div>
                                <div className="mt-11">
                                    <FormControl component="fieldset">
                                        ...
                                        <RadioGroup
                                            row
                                            value={privatee}
                                            onChange={(e) => setPrivatee(Number(e.target.value))}
                                        >
                                            <FormControlLabel
                                                value={1}
                                                control={<Radio />}
                            ...
                            <button
                                ...
                                onClick={handleAddNewExerciseToType}
                            >
                                Add
                            </button>
                        </div>
...
export default AddExerciseToType