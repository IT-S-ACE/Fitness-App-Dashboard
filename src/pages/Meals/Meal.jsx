...
import s1 from '../../assets/Random-Image/image.png'
import s2 from '../../assets/Random-Image/image-1.png'
import s3 from '../../assets/Random-Image/profile.png'
import sEgg from '../../assets/Random-Image/eggs-e1648216350119-500x500.jpeg'
import AddIngredient from '../../components/ADD/Meal/AddIngredient'
                    ...
                    {allIngredients?.ingredient?.map((ing) => {
                        return <>
                            <div key={ing.id} className="bg-black border-1 border-black min-h-56 min-w-52 overflow-hidden w-80 rounded-xl m-5 card-effect">
                                <div className="">
                                    <img className='w-full h-52 ' src={`https://142c-89-39-107-204.ngrok-free.app/Uploads/${ing.image}`} alt="Minecraft" />
                                </div>
                                <div className="p-4">
                    ...
                    {AllMeals?.data?.map((meal) => {
                        return <>
                            <div key={meal.id} className="bg-black border-1 border-black min-h-56 w-64 overflow-hidden rounded-xl m-5 card-effect">
                                <div className="">
                                    <img className='w-full h-52' src={`https://142c-89-39-107-204.ngrok-free.app/Uploads/${meal.image}`} alt="Minecraft" />
                                </div>
                                <div className="p-4">
                                    <strong className="flex justify-between font-extrabold mt-3">
                                        <span className='text-white '>
                                    ...
                                    <div className='text-white ml-1 '>
                                        ...
                                        <div className='flex flex-wrap max-h-14 overflow-hidden'>
                                            {meal.ingredients.map((imag) => {
                                                return (
                                                    <>
                                                        <img src={`https://142c-89-39-107-204.ngrok-free.app/Uploads/${imag.image}`} alt='' className='rounded-2xl w-5 h-5 m-1' />
                                                    </>
                                                )
                                            })}
                                        </div>
